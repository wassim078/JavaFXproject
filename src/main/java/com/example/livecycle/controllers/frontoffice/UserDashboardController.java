...
public class UserDashboardController {
    ...
    private void setActiveButton(Node activeControl) {
        ...
        List<Node> navControls = Arrays.asList(
                ...
                reclamationBtn, panierBtn, forumBtn,commandeBtn
        );
    ...
    public void refreshCartCount() {
        try {
            ...
            int count = panier != null ? new JSONObject(panier.getItems()).length() : 0;
    ...
    public void showPanier(ActionEvent actionEvent) throws IOException {
        refreshCurrentUser();
        setActiveButton(panierBtn);

        FXMLLoader loader = new FXMLLoader(getClass().getResource(
                "/com/example/livecycle/frontoffice/panier.fxml"
        ));

        Parent cartPane = loader.load();

// ✏ *Right here, after loader.load():*
        PanierController cmc = loader.getController();
...
// Now swap it in
    ...
    @FXML
    public void showCommandeManagement(ActionEvent actionEvent) {
        setActiveButton(commandeBtn);
        refreshCurrentUser();
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource(
            ...
            // Initialize the controller with the current user and dashboard
            CommandeController controller = loader.getController();
            controller.setCurrentUser(currentUser);
            controller.setDashboardController(this);

            // Swap the view inside the content area
            contentArea.getChildren().setAll(view);
        } catch (IOException e) {
            e.printStackTrace();
            showLoadError("Commande Management", e);
        }
    }
}